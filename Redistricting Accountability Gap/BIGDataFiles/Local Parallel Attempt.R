...
testfiles <- setNames(as.data.frame(list.files(path = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/ForParallel/output_maps_2/OM1", pattern="*.geojson", full.names = TRUE)), c('Files'))
...
codes <- as.character(testfiles[c(1:236), 1])

# Consider adding in: .export = c("VA_zipCRSmerge")

example <- foreach(i = codes, .combine = rbind, .export = "VA_zipCRSmerge", packages = c("raster", "foreach", "rgdal", "sp")) %dopar% {
  require(sp)
  require(rgdal)
  require(rgeos)
  require(raster)
