...
# Add in the chamber-level data for House 2013 [COME BACK TO AND DETERMINE IF WE WANT TO GET RID OF THIS. NOT SURE I REALLY EVEN GO ON TO USE THESE METRICS]
...
# Calculate absolute value distance of legislator ideal point from mean policy perfernece of his or her district to get metric of accountability (Might need to delete this depending on what is below. Doesn't seem to jive with what I had in the presentations)
VACurMapSub$distrepaccount <- abs(VACurMapSub$mrp_mean-VACurMapSub$np_score)

# Graphics for insertion into various presentations [Can eliminate from ultimate Github posting]
plot(VACurMapSub$distrepaccount, main="House of Delegates Constituency Alignment", ylab = "Constituency Alignment", xlab = "District Number")
polygon(VACurMapSub$distrepaccount, col="red", border="blue")

# Calculate margin of competitiveness (For visualization)
VACurMapSub$PolComp <- abs(VACurMapSub$RepPercent-VACurMapSub$DemPercent)
VACurMapSub$PolComp
ggplot(VACurMapSub, aes(x=house2013, y=PolComp)) +geom_point(aes(col=party))+ labs(y = "Margin", title="Electoral Competition", x = "District")

# Calculate District Averages for Metrics (For Vizualization)
...
# Policy Liberalism Trend (Viz for presentation)
PolLib <- read.csv("ForPolLib.csv", header=T)
PolLibVA <- subset(PolLib, abb == "VA" & year >= 2000 & year <= 2016)
levels(PolLib$abb)
ggplot(data=PolLibVA, aes(x=year, y=median)) + geom_line() + geom_point()  +
  xlab("Year") + ylab("Median Policy") +
  ggtitle("Policy Conservatism Over Time")

#### Aggregate Analysis of All GEOJSON FILES #### 
setwd("C:/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles")
...
library(rgeos)
testfile <- list.files(path = "C:/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/output_maps_2", pattern="*.geojson", full.names = TRUE)
...
#################### For lapply method to get average for each map
setwd("/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/")
testfiles <- list.files(path = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/ForParallel/output_maps_2/OM3", pattern="*.geojson", full.names = TRUE)
...
idepo <- function(filename) {
  ...
  data1 <- data
  dataOM3 <- do.call(rbind.data.frame, data1)
  write.csv(dataOM3, "dataOM3.csv")

dataOM11 <- do.call(rbind.data.frame, data1)
write.csv(dataOM11, "dataOM11.csv")

dataOM10 <- do.call(rbind.data.frame, data1)
write.csv(dataOM10, "dataOM10.csv")

dataOM9 <- do.call(rbind.data.frame, data1)
write.csv(dataOM9, "dataOM9.csv")

dataOM8 <- do.call(rbind.data.frame, data1)
write.csv(dataOM8, "dataOM8.csv")

dataOM8 <- do.call(rbind.data.frame, data1)
write.csv(dataOM7, "dataOM8.csv")

dataOM6 <- do.call(rbind.data.frame, data1)
write.csv(dataOM6, "dataOM6.csv")

dataOM5 <- do.call(rbind.data.frame, data1)
write.csv(dataOM5, "dataOM5.csv")

dataOM4 <- do.call(rbind.data.frame, data1)
write.csv(dataOM4, "dataOM4.csv")

dataOM2 <- do.call(rbind.data.frame, data1)
write.csv(dataOM2, "dataOM2.csv")

dataOM1 <- do.call(rbind.data.frame, data1)
write.csv(dataOM1, "dataOM1.csv")
# Next!
dataOM2 <- do.call(rbind.data.frame, data1)

############Lapply to get average gap between state average and cluster average for each map
setwd("/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/")
testfiles <- list.files(path = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/ForParallel/output_maps_2/OM2", pattern="*.geojson", full.names = TRUE)

idepostategap <- function(filename) {
  precinctmaps1 <- readOGR(dsn = filename, layer = ogrListLayers(filename))
  projection(precinctmaps1) <- projection(VA_zipsubCRSmerge)
  inters1 <- raster::intersect(VA_zipsubCRSmerge, precinctmaps1)
  ...
  Respperzipav1 <- mean(mergefull1$Rescount)
  Zipperdistav1 <- mean(mergefull1$ZCTA5CE00.y)
  Stategap <- abs(mergefull1$IdePoAv.x - mean(mergeddata1$IdePoAv))
  Output1 <- cbind(IdePoAvfu1, IdePoAvse1, Respperzipav1, Zipperdistav1, Stategap)
  print(Output1) 
}

# clusterExport(cl, list("idepo", "projection<-", "VA_zipsubCRSmerge", "area"))
data <- lapply(testfiles, idepostategap)
data2 <- data
statedataOM2 <- do.call(rbind.data.frame, data2)
write.csv(statedataOM2, "statedataOM2.csv")
