...
# Point plot in base R, multicolor scatter of margin per district by party, lapply, functions, Basic line Time Series, Basic Bar with continuous X and Frequency Y with normal curve imposed, importing and using geojson
...
EffGapCalc$EG <- EffGapCalc$netwasted/EffGapCalc$surveyed_total
summary(EffGapCalc$EG)
...
library(rgdal)
testfile <- list.files(path = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/output_maps_2", pattern="*.geojson", full.names = TRUE)
...
data1 <- data
data2 <- cbind(data1[1:117])
data3 <- as.data.frame(data2)
data3$V1 <- as.numeric(data3$V1)
summary(data3$V1)
# The mean efficiency gap for all maps is .033424, or 3.34%
...
seatshare <- function(filename) {
  ...
  aggdata2 <-aggregate(map2, by=list(map2$cluster), 
                      FUN=sum, na.rm=TRUE);
  aggdata2$result <- ifelse(aggdata$reptotal > aggdata$demtotal,1, ifelse(aggdata2$demtotal > aggdata$reptotal,0, NA));
...
seatshare1
# On average, maps produced outcomes where Republicans held 65%, or 65, of the seats, and  Democrats held 35%, or 35 o the seats... Need to make sure we calculated this right as it doesn't really make sense given the efficiency gap estimate. 
propseat1
# Confirmed the above. Will have to assess after the updated maps come in. 
reptot1
# Republicans won 54.00% of the seats statewide
demtot2
# Democrats won 40.31% of the seats statewide
...
# Attaching warshaw ideal_points to VA_zipsubCRS (START HERE)
library(raster)
...
oofmap<- readOGR(dsn = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/output_maps_2/1ebdc363-627b-4baa-9f74-c250ed1928b1.geojson", layer = "1ebdc363-627b-4baa-9f74-c250ed1928b1")
...
filename <- list.files(path = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/ForParallel/output_maps_2/OM1", pattern="*.geojson", full.names = TRUE)
...
idepo <- function(filename) {
  ...
  inters <- intersect(VA_zipsubCRSmerge, precinctmaps1)
  inters$area <- area(inters)
  mergeddata <- as.data.frame(inters@data)
  mergeddata <- mergeddata[!is.na(mergeddata$IdePoAv),]
  mergedistrict1 <-aggregate(mergeddata, by=list(mergeddata$cluster), 
...
# Other New Map Analysis

setwd("/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/output_maps_2")
NewMapsAnalysis <- read.csv("manifest.csv")
NewMapsAnalysis$Map <- as.factor(seq(1,234,1))
names(NewMapsAnalysis)

x <- NewMapsAnalysis$competitive_districts
h<-hist(x, breaks=10, col="red", xlab="Competitive Districts", 
        main="Distribution of Competitive Districts Across Maps") 
xfit<-seq(min(x),max(x),length=40) 
yfit<-dnorm(xfit,mean=mean(x),sd=sd(x)) 
yfit <- yfit*diff(h$mids[1:2])*length(x) 
lines(xfit, yfit, col="blue", lwd=2)
summary(NewMapsAnalysis$competitive_districts)

x <- NewMapsAnalysis$racial_dissimilarity
h<-hist(x, breaks=10, col="blue", xlab="Racial Dissimilarity Index", 
        main="Distribution of Racial Dissimilarity Across Maps") 
xfit<-seq(min(x),max(x),length=40) 
yfit<-dnorm(xfit,mean=mean(x),sd=sd(x)) 
yfit <- yfit*diff(h$mids[1:2])*length(x) 
lines(xfit, yfit, col="red", lwd=2)
summary(NewMapsAnalysis$racial_dissimilarity)

setwd("/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/BIGWork/BIGDataFiles/ForParallel")
effgap34 <- read.csv('Efficiency Gap.csv')
effgap34$X <- as.factor(effgap34$X)
ggplot(effgap34, aes(x=X, y=V1)) +
  geom_point(size=2, shape=20, col = "blue") + theme(axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x=element_blank()) + labs(y = "Efficiency Gap")
