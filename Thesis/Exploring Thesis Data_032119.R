...
# 4. State GDP Dependent Variable Reduced State Set (i.e. state financing vars knock out DC)
fe4 <- lm(scale(GDP) ~ scale(MatchIndex) + scale(PropTax) + scale(Sales.Tax) + scale(Corporate.Income.Tax) + Export.Industry + Manufacturing.Industry + scale(SAEMHI_PT) + scale(SAEPOVRTALL_PT) + scale(AnnualSubsFullState) + scale(AnnualPercoFullState) + scale(Unemployment.insurance.compensation) + scale(Personal.dividend.income) + scale(Monetary.rent) + scale(Population..persons..3.) + scale(Per.capita.unemployment.insurance.compensation.4.) + scale(Farm.proprietors..income) + scale(Average.nonfarm.proprietors..income) + scale(Public.welfare) + scale(Highways) + scale(Health) + factor(StateNumID) + factor(Year), data=FullDataSetForMerge)
...
# To do the manual calculation, we need to get the mean and the N for both 0 and 1 in the treatment var and total for each stratum
# In stata, we do this with bysort strata: tabstat tothrs hrsemp totrain union_hat employ sales avgsal ,by(union) s(mean n) format(%10.0g) Here we can do:

atecalc <- FullDataSetForMerge2 %>% 
  group_by(pq, IncentivesTab) %>% 
  dplyr::summarise(GDP_mean = mean(GDP),
            avg_count = n()) 

# Normalize the values 
atecalc$Normmean <- (atecalc$GDP_mean - min(atecalc$GDP_mean))/(max(atecalc$GDP_mean)-min(atecalc$GDP_mean))

# So then what we'd want to do is /* di ((Str1outcome1 mean - Str1outcome0 Str1mean)*str1outcome1N + (Str2outcome1 mean - Str2outcome0 Str2mean)*str2outcome1N (Str3outcome1 mean - Str3outcome0 Str3mean)*str3outcome1N + (Str4outcome1 mean - Str4outcome0 Str4mean)*str4outcome1N + (Str5outcome1 mean - Str5outcome0 Str4mean)*str5outcome1N

psmATT <- ((atecalc[2,5] - atecalc[1,5]) * atecalc[2,4]) + ((atecalc[4,5] - atecalc[3,5]) * atecalc[4,4]) + ((atecalc[6,5] - atecalc[5,5]) * atecalc[6,4]) + ((atecalc[8,5] - atecalc[7,5]) * atecalc[8,5]) + ((atecalc[10,5] - atecalc[9,5]) * atecalc[10,4])

# So in this case incentives have a pretty negative effect overall...
...
atecalc2 <- FullDataSetForMerge2 %>% 
  group_by(pq, IncentivesTab) %>% 
  dplyr::summarise(GDP_mean = mean(FullStateGDP),
            avg_count = n()) 

# Normalize the values 
atecalc2$Normmean <- (atecalc2$GDP_mean - min(atecalc2$GDP_mean))/(max(atecalc2$GDP_mean)-min(atecalc2$GDP_mean))

psmFuATT <- ((atecalc2[2,5] - atecalc2[1,5]) * atecalc2[2,4]) + ((atecalc2[4,5] - atecalc2[3,5]) * atecalc2[4,4]) + ((atecalc2[6,5] - atecalc2[5,5]) * atecalc2[6,4]) + ((atecalc2[8,5] - atecalc2[7,5]) * atecalc2[8,5]) + ((atecalc2[10,5] - atecalc2[9,5]) * atecalc2[10,4])
...
atecalc3 <- FullDataSetForMerge32 %>% 
  group_by(pq, IncentivesTab) %>% 
  dplyr::summarise(GDP_mean = mean(GDP),
            avg_count = n()) 

# Normalize the values 
atecalc3$Normmean <- (atecalc3$GDP_mean - min(atecalc3$GDP_mean))/(max(atecalc3$GDP_mean)-min(atecalc3$GDP_mean))

psmGfATT <- ((atecalc3[2,5] - atecalc3[1,5]) * atecalc3[2,4]) + ((atecalc3[4,5] - atecalc3[3,5]) * atecalc3[4,4]) + ((atecalc3[6,5] - atecalc3[5,5]) * atecalc3[6,4]) + ((atecalc3[8,5] - atecalc3[7,5]) * atecalc3[8,5]) + ((atecalc3[10,5] - atecalc3[9,5]) * atecalc3[10,4])
...
atecalc4 <- FullDataSetForMerge32 %>% 
  group_by(pq, IncentivesTab) %>% 
  dplyr::summarise(GDP_mean = mean(FullStateGDP),
            avg_count = n()) 

# Normalize the values 
atecalc4$Normmean <- (atecalc4$GDP_mean - min(atecalc4$GDP_mean))/(max(atecalc4$GDP_mean)-min(atecalc4$GDP_mean))

psmFuGfATT <- ((atecalc4[2,5] - atecalc4[1,5]) * atecalc4[2,4]) + ((atecalc4[4,5] - atecalc4[3,5]) * atecalc4[4,4]) + ((atecalc4[6,5] - atecalc4[5,5]) * atecalc4[6,4]) + ((atecalc4[8,5] - atecalc4[7,5]) * atecalc4[8,5]) + ((atecalc4[10,5] - atecalc4[9,5]) * atecalc4[10,4])
...
inctab <- function(inc) {
  ...
  atecalc <- FullDataSetForMerge_Each %>% 
    group_by(pq, IncTabEach) %>% 
    dplyr::summarise(GDP_mean = mean(GDP),
              avg_count = n()) 

  atecalc$Normmean <- (atecalc$GDP_mean - min(atecalc$GDP_mean))/(max(atecalc$GDP_mean)-min(atecalc$GDP_mean))
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "ATT", sep="."), ((atecalc[2,5] - atecalc[1,5]) * atecalc[2,4]) + ((atecalc[4,5] - atecalc[3,5]) * atecalc[4,4]) + ((atecalc[6,5] - atecalc[5,5]) * atecalc[6,4]) + ((atecalc[8,5] - atecalc[7,5]) * atecalc[8,5]) + ((atecalc[10,5] - atecalc[9,5]) * atecalc[10,4]), envir = .GlobalEnv)
...
inctab2 <- function(inc) {
  ...
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "ATTFu", sep="."), ((atecalc[2,5] - atecalc[1,5]) * atecalc[2,4]) + ((atecalc[4,5] - atecalc[3,5]) * atecalc[4,4]) + ((atecalc[6,5] - atecalc[5,5]) * atecalc[6,4]) + ((atecalc[8,5] - atecalc[7,5]) * atecalc[8,5]) + ((atecalc[10,5] - atecalc[9,5]) * atecalc[10,4]), envir = .GlobalEnv)
...
inctab3 <- function(inc) {
  ...
  atecalc <- FullDataSetForMerge3_Each %>% 
    group_by(pq, IncTabEach) %>% 
    dplyr:: summarise(GDP_mean = mean(GDP),
              avg_count = n()) 
  ...
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "ATTGF", sep="."), ((atecalc[2,5] - atecalc[1,5]) * atecalc[2,4]) + ((atecalc[4,5] - atecalc[3,5]) * atecalc[4,4]) + ((atecalc[6,5] - atecalc[5,5]) * atecalc[6,4]) + ((atecalc[8,5] - atecalc[7,5]) * atecalc[8,5]) + ((atecalc[10,5] - atecalc[9,5]) * atecalc[10,4]), envir = .GlobalEnv)
...
inctab4 <- function(inc) {
  ...
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm6.cl.cov1", sep="."), psm.cl.rob(lm(scale(FullStateGDP) ~ scale(Job.Creation.Tax.Credit) + scale(Investment.Tax.Credit) + scale(Research.and.Development.Credit) + scale(Property.Tax.Abatement) + scale(Customized.Job.Training.Subsidy) + scale(PropTax) + scale(Sales.Tax) + scale(Corporate.Income.Tax) + Export.Industry + Manufacturing.Industry + scale(SAEMHI_PT) + scale(SAEPOVRTALL_PT) + scale(AnnualSubsFullState) + scale(AnnualPercoFullState) + scale(Unemployment.insurance.compensation) + scale(Personal.dividend.income) + scale(Monetary.rent) + scale(Population..persons..3.) + scale(Per.capita.unemployment.insurance.compensation.4.) + scale(Farm.proprietors..income) + scale(Average.nonfarm.proprietors..income) + scale(Public.welfare) + scale(Highways) + scale(Health) + factor(StateNumID) + factor(Year), data = FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 1,]), FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 1,]), envir = .GlobalEnv)
  
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm7", sep="."), psmfe_All(FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 2,]), envir = .GlobalEnv)
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm7.cl.cov1", sep="."), psm.cl.rob(lm(scale(FullStateGDP) ~ scale(Job.Creation.Tax.Credit) + scale(Investment.Tax.Credit) + scale(Research.and.Development.Credit) + scale(Property.Tax.Abatement) + scale(Customized.Job.Training.Subsidy) + scale(PropTax) + scale(Sales.Tax) + scale(Corporate.Income.Tax) + Export.Industry + Manufacturing.Industry + scale(SAEMHI_PT) + scale(SAEPOVRTALL_PT) + scale(AnnualSubsFullState) + scale(AnnualPercoFullState) + scale(Unemployment.insurance.compensation) + scale(Personal.dividend.income) + scale(Monetary.rent) + scale(Population..persons..3.) + scale(Per.capita.unemployment.insurance.compensation.4.) + scale(Farm.proprietors..income) + scale(Average.nonfarm.proprietors..income) + scale(Public.welfare) + scale(Highways) + scale(Health) + factor(StateNumID) + factor(Year), data = FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 2,]), FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 2,]), envir = .GlobalEnv)
  
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm8", sep="."), psmfe_All(FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 3,]), envir = .GlobalEnv)
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm8.cl.cov1", sep="."), psm.cl.rob(lm(scale(FullStateGDP) ~ scale(Job.Creation.Tax.Credit) + scale(Investment.Tax.Credit) + scale(Research.and.Development.Credit) + scale(Property.Tax.Abatement) + scale(Customized.Job.Training.Subsidy) + scale(PropTax) + scale(Sales.Tax) + scale(Corporate.Income.Tax) + Export.Industry + Manufacturing.Industry + scale(SAEMHI_PT) + scale(SAEPOVRTALL_PT) + scale(AnnualSubsFullState) + scale(AnnualPercoFullState) + scale(Unemployment.insurance.compensation) + scale(Personal.dividend.income) + scale(Monetary.rent) + scale(Population..persons..3.) + scale(Per.capita.unemployment.insurance.compensation.4.) + scale(Farm.proprietors..income) + scale(Average.nonfarm.proprietors..income) + scale(Public.welfare) + scale(Highways) + scale(Health) + factor(StateNumID) + factor(Year), data = FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 3,]), FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 3,]), envir = .GlobalEnv)
  
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm9", sep="."), psmfe_All(FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 4,]), envir = .GlobalEnv)
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm9.cl.cov1", sep="."), psm.cl.rob(lm(scale(FullStateGDP) ~ scale(Job.Creation.Tax.Credit) + scale(Investment.Tax.Credit) + scale(Research.and.Development.Credit) + scale(Property.Tax.Abatement) + scale(Customized.Job.Training.Subsidy) + scale(PropTax) + scale(Sales.Tax) + scale(Corporate.Income.Tax) + Export.Industry + Manufacturing.Industry + scale(SAEMHI_PT) + scale(SAEPOVRTALL_PT) + scale(AnnualSubsFullState) + scale(AnnualPercoFullState) + scale(Unemployment.insurance.compensation) + scale(Personal.dividend.income) + scale(Monetary.rent) + scale(Population..persons..3.) + scale(Per.capita.unemployment.insurance.compensation.4.) + scale(Farm.proprietors..income) + scale(Average.nonfarm.proprietors..income) + scale(Public.welfare) + scale(Highways) + scale(Health) + factor(StateNumID) + factor(Year), data = FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 4,]), FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 4,]), envir = .GlobalEnv)
  
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm10", sep="."), psmfe_All(FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 5,]), envir = .GlobalEnv)
  
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "psm10.cl.cov1", sep="."), psm.cl.rob(lm(scale(FullStateGDP) ~ scale(Job.Creation.Tax.Credit) + scale(Investment.Tax.Credit) + scale(Research.and.Development.Credit) + scale(Property.Tax.Abatement) + scale(Customized.Job.Training.Subsidy) + scale(PropTax) + scale(Sales.Tax) + scale(Corporate.Income.Tax) + Export.Industry + Manufacturing.Industry + scale(SAEMHI_PT) + scale(SAEPOVRTALL_PT) + scale(AnnualSubsFullState) + scale(AnnualPercoFullState) + scale(Unemployment.insurance.compensation) + scale(Personal.dividend.income) + scale(Monetary.rent) + scale(Population..persons..3.) + scale(Per.capita.unemployment.insurance.compensation.4.) + scale(Farm.proprietors..income) + scale(Average.nonfarm.proprietors..income) + scale(Public.welfare) + scale(Highways) + scale(Health) + factor(StateNumID) + factor(Year), data = FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 5,]), FullDataSetForMerge3_Each[FullDataSetForMerge3_Each$pq == 5,]), envir = .GlobalEnv)
  ...
  assign(paste(substr(deparse(substitute(inc)), 21, nchar(deparse(substitute(inc)))), "ATTFUGG", sep="."), ((atecalc[2,5] - atecalc[1,5]) * atecalc[2,4]) + ((atecalc[4,5] - atecalc[3,5]) * atecalc[4,4]) + ((atecalc[6,5] - atecalc[5,5]) * atecalc[6,4]) + ((atecalc[8,5] - atecalc[7,5]) * atecalc[8,5]) + ((atecalc[10,5] - atecalc[9,5]) * atecalc[10,4]), envir = .GlobalEnv)
...
# Manuually add in the ATTs calculates along the way - I probably calculated these wrong. 
print(psmATT)
print(Job.Creation.Tax.Credit.ATT)
print(Investment.Tax.Credit.ATT)
print(Research.and.Development.Credit.ATT)
print(Property.Tax.Abatement.ATT)
print(Customized.Job.Training.Subsidy.ATT)
...
stargazer(reg1, reg3, reg5, reg7, fe1, fe3, fe5, fe7, lteffreg1, lteffreg3, lteffreg5, lteffreg7, se=list(reg1.rse[,"Std. Error"], reg3.rse[,"Std. Error"], reg5.rse[,"Std. Error"], reg7.rse[,"Std. Error"], fe1.cl.robust.se.1, fe3.cl.robust.se.1, fe5.cl.robust.se.1, fe7.cl.robust.se.1, lteffreg1.rse[,"Std. Error"], lteffreg3.rse[,"Std. Error"], lteffreg5.rse[,"Std. Error"], lteffreg7.rse[,"Std. Error"]), type="html", dep.var.labels=c("State-Industry GDP"), omit = c("Year", "StateNumID", "Sales.Tax", "Corporate.Income.Tax", "PropTax", "SAEMHI_PT", "SAEPOVRTALL_PT", "AnnualSubsFullState", "AnnualPercoFullState", "Unemployment.insurance.compensation", "Personal.dividend.income", "Monetary.rent", "Population..persons..3.", "Per.capita.unemployment.insurance.compensation.4.", "Farm.proprietors..income", "Average.nonfarm.proprietors..income", "avgpropertytax", "avgsalestax", "avgcit", "Export.Industry", "Manufacturing.Industry", "Highways", "Health", "Public.welfare", "Constant"), covariate.labels=c("All Incentives", "Job Creation Tax Credit", "Investment Tax Credit", "Research and Development Credit", "Property Tax Abatement", "Customized Job Training Subsidy", "Avg. All Incentives", "Avg. Job Creation Tax Credit", "Avg. Investment Tax Credit", "Avg. Research and Development Credit", "Avg. Property Tax Abatement", "Avg. Customized Job Training Subsidy"), out="stateindgdpmodels.htm", style = "aer", title = "TABLE 2: Two-Way fixed Effects regression results analyzing economic incentives and their impact on State-Industry GDP", add.lines = list(c("State fixed effects", rep("NO", 4), rep("YES", 4), rep("NO", 4)), c("Time fixed effects", rep("NO", 4), rep("YES", 4), rep("NO", 4)), c("Government Finance Controls", "Yes", "No", "Yes", "No", "YES", "NO", "YES", "NO", "YES", "NO", "YES", "NO"), c("Robust Standard Errors", rep("YES", 4), rep("NO", 4), rep("YES", 4)), c("Clustered Standard Errors", rep("NO", 4), rep("YES", 4), rep("NO", 4))), column.labels = c(rep("Basic OLS", 4), rep("Two-Way Fixed Effects", 4), rep("Long-Term Effects", 4)))
# If I end up wanting to omit any vars from the output - omit = c("SAEMHI_PT", "SAEPOVRTALL_PT")
