...
Sys.setenv(PATH = paste(Sys.getenv("PATH"), "/Users/chris/AppData/Roaming/Microsoft/Windows/Start Menu/Programs/MiKTeX 2.9", sep=.Platform$path.sep))
...
# Test for Multicollinearity - Does not work 
library(car)
vif(my.pdm.train.lag)

# Regular LM using zip dummies
my.pdm.train.lag.lm <- lm (rentprice ~ lag(rentprice, 1) + starsav + is_openave + funnyav + coolav + usefulav + Number_of_reviews + postal_code, data = Full_data_long_test, na.action = na.omit) 
summary(my.pdm.train.lag.lm)

# Predict 
#LM
Full_data_long_test$pred.plm.test.lag <- predict(my.pdm.train.lag.lm, Full_data_long_test, interval = "prediction")
...
# Built random effects model on train
my.pdm.train.food.lag <- plm(data = Full_data_long_food_train, 
                        formula = my.lag.formula, 
                        model = 'random',
                        index = my.index)
summary(my.pdm.train.food.lag)

# Predict 
Full_data_long_food_test$pred.plm.test.lag <- predict(my.pdm.train.food.lag, Full_data_long_food_test, type='response')
...
# Built random effects model on train
my.pdm.train.impute.lag.food <- plm(data = data10_train, 
                               formula = my.formula.impute.lag.food, 
                               model = 'random',
                               index = my.index)
