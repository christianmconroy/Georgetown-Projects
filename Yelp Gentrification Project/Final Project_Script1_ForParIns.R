...
testfiles <- list.files(path = "/Users/chris/Documents/GeorgetownMPPMSFS/McCourtMPP/Semester4MPP/DataScienceIntro/dataset", pattern="*.json", full.names = TRUE)
review <- testfiles[3]

YelpDataSet <- foreach(i = review, .combine = rbind) %dopar% { 
  
  files <- jsonlite::stream_in(file(i))
...
unique <- unique(yelp_business$business_id)
...
jsonlite::stream_in(file("dataset/review.json"), handler = function(df) {
  df <- dplyr::filter(df, business_id %in% unique)
...
write.csv(yelp_review, "yelpreview_full.csv")
write.csv(mydata, "yelpreview_pho.csv")

yelp_review <- yelp_review[yelp_review$date >= "2010-11-01" & yelp_review$date <= "2017-12-31"]
...
yelp_review <- inner_join(yelp_review, yelp_business, by='business_id', match='all')
