...
checkinzip <- checkinbiz %>% 
  group_by(postal_code) %>% 
  summarize(Friavmean = mean(Friday_ave),
            Monavmean = mean(Monday_ave),
            Satavmean = mean(Saturday_ave),
            Sunavmean = mean(Sunday_ave),
            Thuravmean = mean(Thursday_ave),
            Tuesavmean = mean(Tuesday_ave),
            Wedavmean = mean(Wednesday_ave),
            FriTotal = sum(Friday_total),
            MonTotal = sum(Monday_total),
            SatTotal = sum(Saturday_total),
            SunTotal = sum(Sunday_total),
            ThurTotal = sum(Thursday_total),
            TuesTotal = sum(Tuesday_total),
            WedTotal = sum(Wednesday_total))
...
# Imputation
sapply(Full_data_long, function(x) sum(is.na(x)))
Imputed_Full_data_long <-amelia(Full_data_long,m=3,ts= 'time', cs= 'postal_code', p2s=0, intercs = FALSE, idvars=c('City', 'State', 'Metro', 'CountyName', 'year'))
Imputed_Full_data_long
plot(Imputed_Full_data_long, which.vars = 7)

# Do we need polytime or setting the power of polynomial for time effects (integer between 0 and 3)

write.amelia(obj=Imputed_Full_data_long, file.stem="imputedfull", separate = FALSE)
Imputefull <- read.csv('imputedfull.csv')
names(Imputefull)

# Create train and test set using the last 12 months (1 year) for the test set 
Imputefull$time <- as.Date(Imputefull$time, "%Y-%m-%d")
Imputefull_train <- Imputefull[Imputefull$time < "2017-01-01",]
Imputefull_test <- Imputefull[Imputefull$time >= "2017-01-01",]

# Build the mixed effects model (Can I do this before the RF or do I have to do it after somehow?)
# Hausman Test
# set options for Hausman test
my.formula2 <- rentprice ~ starsav + starssd + is_openave + funnyav + coolav + usefulav
# Had too take standard deviation out because of all the NAs. What to do? Can impute those too?  

my.index <- c('postal_code','time')

# Conduct Hausman Test
my.hausman.test.train2 <- phtest(x = my.formula2, 
                                data = Imputefull,
                                model = c('within', 'random'),
                                index = my.index)
# print result
print(my.hausman.test.train2)

# High p-value means random is the way to go here. 

# Built random effects model on train
my.pdm.train2 <- plm(data = Imputefull_train, 
                    formula = my.formula, 
                    model = 'random',
                    index = my.index)
summary(my.pdm.train2)
